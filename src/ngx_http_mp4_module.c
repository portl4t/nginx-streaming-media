...
#define NGX_HTTP_MP4_MTYPE_VIDEO  1
#define NGX_HTTP_MP4_MTYPE_AUDIO  2
...
typedef struct {
    ...
    uint32_t              chunks;
    uint32_t              mtype;            // video or audio

    ngx_uint_t            start_sample;     // 从0开始计
...
typedef struct {
    ...
    double                rs;
    double                rate;         // 跳转到的视频关键帧位置相对于整体来讲的百分比
...
static ngx_uint_t ngx_http_mp4_find_key_frame(ngx_uint_t sample, ngx_http_mp4_trak_t *trak);
...
ngx_http_mp4_read_hdlr_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size)
{
    ...
    if (memcmp(atom_header + 16, "vide", 4) == 0) {
        trak->mtype = NGX_HTTP_MP4_MTYPE_VIDEO;

    } else if (memcmp(atom_header + 16, "soun", 4) == 0) {
        trak->mtype = NGX_HTTP_MP4_MTYPE_AUDIO;
    }
...
{
    size_t                 atom_size;
    uint32_t               entries, count, duration, total, start_count;
    uint64_t               start_time;
    ngx_buf_t             *atom, *data;
    ngx_uint_t             start_sample, old_sample, key_sample;
    ngx_mp4_stts_atom_t   *stts_atom;
    ...
    entries = trak->time_to_sample_entries;                             // 条目总数
    start_time = (uint64_t) mp4->start * trak->timescale / 1000;        // 结果等于用户要跳过多少个tick
    if (mp4->rs > 0) {
        start_time = (uint64_t) (mp4->rs * trak->timescale);            // 重新计算开始的tick数目, 针对音频
    }

    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "time-to-sample start_time:%uL", start_time);

    start_sample = 0;                                                   // 保存最终从哪个sample开始, 从0号开始计算
    entry = (ngx_mp4_stts_entry_t *) data->pos;
    end = (ngx_mp4_stts_entry_t *) data->last;

    if (trak->time_to_sample_entries == 1) {
        total = ngx_mp4_get_32value(entry->count);

        if (mp4->rate > 0) {        // 音频处理
            start_count = (uint32_t)(mp4->rate * total);
        }
    }

    while (entry < end) {
        count = ngx_mp4_get_32value(entry->count);                      // 有相同duration的连续sample数目
        duration = ngx_mp4_get_32value(entry->duration);                // duration值(可以理解为tick总数)

        ngx_log_debug2(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                       "count:%uD, duration:%uD", count, duration);

        if (start_count > 0) {                                          // 音频常规情况
            ...
            if (start_time < (uint64_t) count * duration) {                 // 如果成立, 说明就在这组sample中
                start_sample += (ngx_uint_t) (start_time / duration);       // 把还要过掉的sample数加上
                count -= (uint32_t) (start_time / duration);                // 这组sample还要保留的个数, 从1开始算的
...
found:
    ...
    key_sample = ngx_http_mp4_find_key_frame(start_sample, trak);       // 查找关键帧号码, 关键帧是从1开始算的

    if (old_sample != key_sample) {                                     // 代表向前找到了关键帧号码
        start_sample = key_sample - 1;                                  // 由于关键帧是从1开始计数, 所以这里减1, start_sample是从0开始计数的
        mp4->rs = (double)start_sample*1000/(double)trak->timescale;    // 重新计算开始时间

        if (trak->time_to_sample_entries == 1) {
            mp4->rate = (double)start_sample/(double)total;             // 计算一个百分比
        }
    ...
    while (entry < end) {
        ...
        if (start_sample < count) {
            count -= start_sample;                          // 剩余sample的个数
            ngx_mp4_set_32value(entry->count, count);
            break;
        }
...
found:
    ...
    while (entry < end) {
        sample = ngx_mp4_get_32value(entry);
        sample -= start_sample;
        ngx_mp4_set_32value(entry, sample);     // 重新设置关键帧序列号
        entry++;
    }
    ...
    ngx_http_mp4_trak_t *trak)
{
    ...
    if (data) {
        ...
        data->pos += trak->start_sample * sizeof(uint32_t);         // 这里要回退一个? 因为关键帧号是从1开始的
...
static ngx_uint_t
ngx_http_mp4_find_key_frame(ngx_uint_t start_sample, ngx_http_mp4_trak_t *trak)
{
    uint32_t        sample, prev_sample, *entry, *end;
    ngx_buf_t       *data;
    ...
    entry = (uint32_t *) data->pos;
     ...
     while (entry < end) {
         sample = ngx_mp4_get_32value(entry);        // 获得帧号码
